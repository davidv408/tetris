...
export function pickNewShape(): ActiveShape {
  ...
  function clearActiveShape(activeShape: ActiveShape, currentBoard: Board): Board {
    ...
    for (let row = 0; row < currentBoard.length; row++) {
        ...
        for(let col = 0; col < currentBoard[row].length; col++) {
            ...
            if(row >= r0 && row < r0 + shape.length &&
               col >= c0 && col < c0 + shape[0].length && activeShape.shape[row-r0][col-c0] === 1
            ) {
  ...
  export function canDrawActiveShapeAtPosition(activeShape: ActiveShape, newPosition: Position, currentBoard: Board): boolean {
    const shape = activeShape.shape;
    const {r0: newR0, c0: newC0} = newPosition;
    
    // Check if draw at the new position will out of bounds
    if(newR0 + shape.length > currentBoard.length || newC0 < 0 || newC0 + shape[0].length > currentBoard[0].length) {
      return false
    }
  ...
  export function drawActiveShapeAtPosition(activeShape: ActiveShape, newPosition: Position, currentBoard: Board): Board {
    const shape = activeShape.shape;
    ...
    for (let row = 0; row < newBoard.length; row++) {
        for(let col = 0; col < newBoard[row].length; col++) {
          ...
          if(newBoard[row][col].value === 1 || !(row >= r0 && row < r0 + shape.length && col >= c0 && col < c0 + shape[0].length)) {
  ...
  export function canDrawActiveShapeAsNewShape(activeShape: ActiveShape, newShape: number[][], currentBoard: Board): boolean {
     const {r0, c0} = activeShape.position;

    const newBoard: Board = clearActiveShape(activeShape, currentBoard);

    // Check if draw at the new position will out of bounds
    if(r0 + newShape.length > currentBoard.length || c0 + newShape[0].length > currentBoard[0].length) {
      return false
    }
  ...
  export function drawActiveShapeAsNewShape(activeShape: ActiveShape, newShape: number[][], currentBoard: Board): Board {
    const {r0, c0} = activeShape.position;
    ...
    for (let row = 0; row < newBoard.length; row++) {
        for(let col = 0; col < newBoard[row].length; col++) {
          // If current board already has cell initialized or cell is not part of the active shape, then use that cell value in the new board.
          if(newBoard[row][col].value === 1 || !(row >= r0 && row < r0 + newShape.length && col >= c0 && col < c0 + newShape[0].length)) {
            newBoard[row][col] = {...newBoard[row][col]};
          } else {
              const cell = {
